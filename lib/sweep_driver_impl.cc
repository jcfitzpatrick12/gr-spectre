...
sweep_driver::sptr sweep_driver::make(float min_freq,
                                      float max_freq,
                                      float freq_step,
                                      int samp_rate,
...
sweep_driver_impl::sweep_driver_impl(float min_freq,
                                     float max_freq,
                                     float freq_step,
                                     int samp_rate,
                                     int samples_per_step,
...
{
    // Declare the message port in the constructor
...
void sweep_driver_impl::publish_retune_command()
{
    ...
    retune_command =
        pmt::dict_add(retune_command, _retune_cmd_name, pmt::from_float(_current_freq));
